...
kineval.initRobotJoints = function initRobotJoints() {
    ...
    //   additionally specify parent and child joints for each link
        var parent_link = robot.joints[x].parent,
            ...
            joint = x;
        robot.links[parent_link].children.push(joint);
        robot.links[child_link].parent.push(joint);
    }
    // for (x in robot.links)
    //     console.log("link: ",x,"  |  parent: ",robot.links[x].parent,"  |  child: ",robot.links[x].children);
}
