...
(function ($) {
  ...
  var clProjectOverlay = function () {
    ...
    function updateOverlay(index) {
      ...
      // Update nav buttons
      if (currentIndex <= 0) {
        $('#overlay-prev').addClass('disabled');
      } else {
        $('#overlay-prev').removeClass('disabled');
      }

      if (currentIndex >= $cards.length - 1) {
        $('#overlay-next').addClass('disabled');
      } else {
        $('#overlay-next').removeClass('disabled');
      }
    }

    // open overlay on card click
    $(document).on('click', '.project-card', function () {
      updateOverlay($cards.index(this));
...
class TypewriterEffect {
  ...
  type() {
    ...
    this.element.innerHTML = `<span class="wrap">${this.txt}</span><span class="cursor">|</span>`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    if (textElement) {
      ...
      new TypewriterEffect('changing-text', ['learning.', 'building.', 'moving!'], {
        typeSpeed: 150,
        deleteSpeed: 100,
